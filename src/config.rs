...
pub struct AccountConfig {
    ...
    /// Token ID, status, or user email observed when the token was verified
    pub identity: String,
    /// Cloudflare account name observed when added
...
pub fn config_dir() -> PathBuf {
    if let Some(dir) = std::env::var_os("CLOUDFLARE_CONFIG_DIR").filter(|v| !v.is_empty()) {
        return PathBuf::from(dir);
    }

    #[cfg(windows)]
    let dir = std::env::var_os("APPDATA").map(PathBuf::from).unwrap_or_default().join("cloudflare-cli");

    #[cfg(not(windows))]
    let dir = {
        let home = std::env::var_os("HOME").map(PathBuf::from).unwrap_or_default();
        if cfg!(target_os = "macos") {
            home.join("Library").join("Application Support").join("cloudflare-cli")
        } else {
...
pub fn load() -> Result<Config> {
    ...
    serde_norway::from_str::<Option<Config>>(&text).map(Option::unwrap_or_default).map_err(|e| {
        Error::other(format!("{} is not valid.", path.display()))
            .detail(e.to_string())
            .fix(format!("Fix or delete {}, then run: cloudflare accounts add <name>", path.display()))
    })
}
        ...
        match file.try_lock() {
            ...
            Err(fs::TryLockError::WouldBlock) => {
                return Err(Error::other(format!(
                    "Timed out waiting for the lock at {}. Another cloudflare process may be stuck.",
                    path.display()
                )));
            }
...
mod tests {
    ...
    fn reads_yaml_layout() {
        ...
        assert_eq!(name, "Work");
        assert_eq!(acct.account_id, "acc_123");
        assert_eq!(acct.identity, "user@example.com");
        assert_eq!(acct.account_name, "My Team");
    }
}
