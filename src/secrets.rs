...
//! refuses to start rather than silently writing a file - `CLOUDFLARE_ALLOW_PLAINTEXT_STORE=1` is
...
const SERVICE: &str = "cloudflare-cli";
...
/// The backend for this process. Cached: changing `CLOUDFLARE_SECRET_STORE` mid-process has no
...
fn build() -> Result<Store> {
    if let Some(forced) = std::env::var("CLOUDFLARE_SECRET_STORE").ok().filter(|s| !s.trim().is_empty()) {
        return match forced.to_lowercase().as_str() {
            ...
            _ => Err(Error::invalid(format!(
                "CLOUDFLARE_SECRET_STORE='{forced}' is not a backend available on this platform."
            ))
            .fix("Unset CLOUDFLARE_SECRET_STORE, or set it to one of: dpapi, keychain, libsecret, plaintext.")),
        };
    }
...
fn fallback(reason: &str) -> Result<Store> {
    if std::env::var("CLOUDFLARE_ALLOW_PLAINTEXT_STORE").as_deref() == Ok("1") {
        return Ok(Store::Plaintext);
    }
    Err(Error::other("No secure credential store is available on this machine.").detail(reason).fix(
        "Install a keystore, or set CLOUDFLARE_ALLOW_PLAINTEXT_STORE=1 to store API tokens in a 0600 file instead.",
    ))
}
...
fn warn_plaintext() {
    ...
    WARNED.get_or_init(|| {
        eprintln!(
            "warning: API tokens are stored unencrypted in {} (CLOUDFLARE_ALLOW_PLAINTEXT_STORE=1).",
    ...
    pub fn unprotect(data: &[u8], path: &Path) -> Result<Vec<u8>> {
        ...
        if ok == 0 {
            return Err(Error::other("The stored API tokens could not be decrypted.")
                ...
                .fix(format!("Delete {} and run: cloudflare accounts add <name>", path.display())));
        }
        Ok(take(out))
    }
}
